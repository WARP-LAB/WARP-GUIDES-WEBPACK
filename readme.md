...
* As of writing webpack version is 3.10.0. **Not all chapters are corrected for webpack 3.x, some are still webpack 2.x.**
* Examples are for frontend only.
* I wrote this and tested locally on macOS and used `nginx` to serve `public` directory with wildcarding routes, under `test` TLD (`webpacktest-chaptername.test` in this case).
...
Make sure you have latest LTS, `node` version 8.x+, `npm` version 5.x+
